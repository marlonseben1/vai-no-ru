...
import { buildWhereClause } from './reserva.helpers';
...
export const ReservaService = {
  async marcarAgendada(reservaId: string): Promise<void> {
    await prisma.$transaction(async (tx) => {
      await tx.schedules.update({
        where: { id: reservaId },
        data: { status: RESERVA_STATUS.AGENDADA, processado: 1 },
      });
      await registrarHistorico(tx, reservaId, 'agendada');
    });
  },
      ...
      await prisma.$transaction(async (tx) => {
        await tx.schedules.update({
          ...
          data: {
            ...
            status: RESERVA_STATUS.NAO_AGENDADA,
            processado: 1,
          },
        });
        await registrarHistorico(tx, reservaId, 'nao_agendada');
      });
      return;
    }
  ...
  async cancelarReserva(
    ...
    | { success: true }
    | { success: false; reason: 'not_found' | 'cannot_cancel' }
  > {
  ...
  async processReserva(body: RuUpfData, userId: string): Promise<void> {
    await prisma.$transaction(async (tx) => {
